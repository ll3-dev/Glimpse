...
## 해결 목표
**현재:** `Apple Intelligence → Local LLM → BYOK → Stub` 우선순위는 있으나, Apple/Local LLM이 실SDK 없이 fallback 위주로 동작함  
**목표:** Apple Intelligence와 Local LLM에 실제 SDK를 연결하고, 라우터에서 실제 생성 결과를 받도록 활성화함

## 성공 기준
- [ ] `local-llm-provider.ts`가 실제 llama SDK 호출을 통해 `summary/tags`를 생성한다.
- [ ] `apple-provider.ts`가 네이티브 브리지를 통해 실제 Apple Intelligence 결과를 생성한다(미지원 기기에서는 정상 fallback).
...
| A2-8 | 10m | A2-7 | 브리지 에러 매핑(`Result<T>`) 및 안전 fallback 구현 | 모듈 미탑재 시 graceful 실패 |
...
| A2-11 | 10m | A2-10 | availability 체크 강화(기기/OS/권한 상태) | 미지원 환경에서 즉시 fallback |
...
| ID | 예상 | 선행 | 작업 | 완료조건 |
|---|---|---|---|---|
| R3-1 | 10m | L1-9, A2-12 | `metadata/router.ts`에서 Apple/Local readiness 조건 재검토 | provider 선택 우선순위 유지 |
| R3-2 | 10m | R3-1 | Apple 실패 시 Local로, Local 실패 시 Stub으로 fallback 보장 | fallback 체인 회귀 없음 |
...
| T4-4 | 10m | T4-3 | `apple-provider.test.ts` 실패 케이스(미지원/네이티브 오류) | fallback/에러 검증 |
| T4-5 | 10m | R3-2 | `router.test.ts` 우선순위/체인 회귀 테스트 보강 | Apple→Local→Stub 순서 보장 |
...
- Apple Intelligence 기기 제약: availability 체크 + Local/Stub fallback 강제
