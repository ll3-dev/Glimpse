...
- 상태: 완료 — 모바일·데스크톱 무검수 연결 UX와 네이티브 데스크톱 조작 검증 포함
...
**Goal:** 모바일과 데스크톱에서 새 연결을 별도 승인 없이 바로 보고, 검색·상세·그래프
사이를 오가며 선택한 지식의 1-hop 관계와 연결 근거를 탐색할 수 있게 한다. 사용자는
잘못된 연결을 발견했을 때만 숨기기 피드백을 남긴다.
...
3. `오늘의 발견` 카드에 두 제목, 근거, 상세 이동을 제공한다. pending은 별도 수락 없이
   즉시 그래프에 표시하고 accepted 폴백도 같은 읽기 흐름을 사용한다.
4. 엣지 탭은 근거와 양 endpoint 상세 이동을 보여주며, 잘못된 연결에만 단일
   `이 연결 숨기기` 액션을 제공한다.
...
2. 발견 카드는 모바일과 같은 무검수 읽기 흐름을 사용하고, 단일 숨기기 mutation만
   `ignored/ignore` 피드백으로 연결한다.
