...
class GlimpsecorePackage : BaseReactPackage() {
  companion object {
    val JNI_PREPARE_MODULE_NAME = setOf(
      "__crabyGlimpseCore_JNI_prepare__"
    )
  }

  init {
    SoLoader.loadLibrary("cxx-glimpsecore")
  }

  override fun getModule(name: String, reactContext: ReactApplicationContext): NativeModule? {
    if (name in JNI_PREPARE_MODULE_NAME) {
      nativeSetDataPath(reactContext.filesDir.absolutePath)
      return GlimpsecorePackage.TurboModulePlaceholder(reactContext, name)
    }
    return null
  }
