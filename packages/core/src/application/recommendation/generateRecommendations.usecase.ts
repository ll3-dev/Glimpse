...
import { appError, isFailure, runEffectSuccess, type AppError, tryPromise } from '../../foundation/effect-result';
import type {
    ...
    const program = Effect.gen(function* () {
      ...
      for (let i = 0; i < items.length; i++) {
        for (let j = i + 1; j < items.length; j++) {
          ...
          const overlap = calculateTagOverlap(deps.coreClient, itemA, itemB);
          if (overlap > 0) {
            candidates.push({
              itemA,
              itemB,
