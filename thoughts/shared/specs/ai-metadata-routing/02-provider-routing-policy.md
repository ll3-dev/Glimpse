...
## 문제
Apple/Local/BYOK를 모두 지원하려면 선택 기준과 실패 처리 기준이 코드 곳곳에 흩어지지 않게 중앙 라우터 정책으로 고정되어야 합니다.

## 해결 목표
**현재:** provider 선택 로직이 일관되게 명시되어 있지 않습니다.  
**목표:** 사용자가 선택한 provider를 기준으로 하는 단일 라우터 정책을 정의합니다.

## 성공 기준
- [ ] 라우팅 입력(플랫폼/설정/콘텐츠)과 출력(선택 provider/메타데이터)이 정의된다.
- [ ] provider 실패 시 다른 provider를 호출하지 않는 기준이 문서화된다.
- [ ] `default` 모드에서만 스텁 provider를 사용하는 규칙이 포함된다.
