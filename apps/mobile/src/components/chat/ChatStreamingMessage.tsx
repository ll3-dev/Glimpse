import { View } from 'react-native';
import { Text } from '@glimpse/ui/primitives';
import { ChatMarkdown } from './ChatMarkdown';
import { parseChatMessageContent } from '@/src/features/chat';

type ChatStreamingMessageProps = {
  content: string;
};

export function ChatStreamingMessage({ content }: ChatStreamingMessageProps) {
  const parsedContent = parseChatMessageContent(content);

  return (
    <View className="mb-3 flex-row justify-start">
      <View className="max-w-[85%] rounded-2xl rounded-bl-md bg-app-surface border border-app-border px-4 py-3 shadow-xs">
        {content ? (
          parsedContent.answer ? (
            <>
              {parsedContent.reasoning && (
                <View className="mb-3 rounded-md bg-app-bg border border-app-border px-3 py-2">
                  <Text className="text-xs font-semibold uppercase tracking-tight text-app-muted">
                    {parsedContent.isReasoningInProgress ? '사고 중' : '사고 요약'}
                  </Text>
                  <Text className="mt-1 text-sm text-app-text" numberOfLines={2}>
                    {parsedContent.reasoningSummary ?? '생각을 정리하고 있습니다.'}
                  </Text>
                </View>
              )}
              <ChatMarkdown
                content={parsedContent.answer}
                textClassName="text-app-text text-base leading-6"
                mutedTextClassName="text-app-muted text-sm leading-5"
              />
            </>
          ) : (
            <View className="rounded-md bg-app-bg border border-app-border px-3 py-2">
              <Text className="text-xs font-semibold uppercase tracking-tight text-app-muted">
                사고 중
              </Text>
              <Text className="mt-1 text-sm text-app-text" numberOfLines={2}>
                {parsedContent.reasoningSummary ?? '생각을 정리하고 있습니다.'}
              </Text>
            </View>
          )
        ) : (
          <Text className="text-app-muted text-sm">생각 중...</Text>
        )}
      </View>
    </View>
  );
}
