...
// 콜백은 예외를 밖으로 던지면 안 된다: Rust 프레임을 통과하는 외국 예외은
// Rust 가 잡을 수 없어 프로세스 abort다 ("C-unwind" ABI 계약).
...
/// JS 런타임 스레드의 CallInvoker가 큐를 drain해 per-name JS 콜백으로
/// 전달한다. CallInvoker가 없는 호스트(Expo Go/폴백)는 `__rustraNative`의
/// `drainEvents()` HostFunction로 폴링 drain할 수 있다.
///
/// 스레딩 계약:
/// - `onRustEvent` (FFI 콜백) — 어느 스레드에서든. 뮤텍스로 보호된 큐에
///   적재만 하고 JS 객체를 건드리지 않는다.
/// - `drain` — 반드시 JS 런타임 스레드에서. `Function::call`은 JS 스레드에서만
///   안전하다 (CallInvoker 콜백 내부 또는 JS가 drainEvents()를 호출할 때).
/// - 큐는 고정 용량(1024) drop-oldest — JS가 느려도 emit 스레드를 블록하지
...
class EventDispatcher : public std::enable_shared_from_this<EventDispatcher> {
 public:
  ...
  /// FFI C 콜백 — emitting 스레드에서 호출된다. 큐 적재 + CallInvoker로
  /// drain 예약만 한다.
 ...
 private:
  ...
  /// per-name JS 콜백 레지스트리 — drain에서만 접근(JS 스레드).
...
/// Android(CallInvokerHolder) 플랫폼 글루가 각자의 방식으로 CallInvoker를
/// 얻어 이 진입점으로 넘긴다.
///
/// CallInvoker 타입은 `facebook::react::CallInvoker`이지만 이 헤더는
/// React-callinvoker 헤더에 의존하지 않는다 — 플랫폼 글루가
/// `void` shared_ptr로 type-erase해서 전달하고 .cpp가 내부에서
/// static_cast로 복원한다(단일 정의 지점 유지).
